...
class EdgeDegreeEmbeddingPCD(torch.nn.Module):
    """ Replace the nn.Embedding layer with a nn.Linear layer for pcd encoding.
    ...
    def __init__(
        ...
        self.SO3_rotation = SO3_rotation
        self.mappingReduced = mappingReduced
        self.inv_feature_dim = inv_feature_dim
        ...
        if self.use_atom_edge_embedding:
            ...
            # nn.init.uniform_(self.source_embedding.weight.data, -0.001, 0.001)
            # nn.init.uniform_(self.target_embedding.weight.data, -0.001, 0.001)
            self.source_embedding = nn.Linear(self.inv_feature_dim, self.edge_channels_list[-1])
            self.target_embedding = nn.Linear(self.inv_feature_dim, self.edge_channels_list[-1])
            self.edge_channels_list[0] = self.edge_channels_list[0] + 2 * self.edge_channels_list[-1]
        else:
            self.source_embedding, self.target_embedding = None, None
    ...
    def forward(
        ...
        # Compute the sum of the incoming neighboring messages for each target node
        x_edge_embedding._reduce_edge(edge_index[1], inv_feature.shape[0])
        x_edge_embedding.embedding = x_edge_embedding.embedding / self.rescale_factor
